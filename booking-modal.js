...
var BookingModal = (function() {
  ...
  // ── HTML ──
  function getHTML() {
    return '<div id="bm-overlay" style="display:none;position:fixed;inset:0;z-index:9000;background:rgba(46,56,77,0.45);backdrop-filter:blur(6px);align-items:center;justify-content:center;padding:20px;overflow-y:auto" onclick="BookingModal._bgClose(event)">' +
      '<div id="bm-modal" style="background:white;border-radius:28px;width:100%;max-width:480px;padding:32px;position:relative;box-shadow:0 24px 80px rgba(46,56,77,0.2);margin:auto">' +
      '<style>' +
        '#bm-modal .bm-row { display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:12px; }' +
        '#bm-modal .bm-row-dates { display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:12px; }' +
        '#bm-modal input[type="date"] { min-width:0; width:100%; max-width:100%; box-sizing:border-box; min-height:46px; line-height:1.5; display:block; }' +
        '@media(max-width:600px){' +
          '#bm-overlay { padding:0; align-items:flex-start; }' +
          '#bm-modal { border-radius:0; min-height:100vh; padding:20px 16px 40px; box-shadow:none; }' +
          '#bm-modal .bm-row { grid-template-columns:1fr; }' +
          '#bm-modal .bm-row-dates { grid-template-columns:1fr; }' +
        '}' +
      '</style>' +
        ...
        '<div id="bm-form-screen">' +
          ...
          '<div class="bm-row">' +
            ...
            '<div><label style="font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:#6B7A99;display:block;margin-bottom:6px">Телефон <span style="color:#E05252">*</span></label>' +
              '<input id="bm-phone" type="tel" placeholder="+7(___) ___-__-__" style="width:100%;box-sizing:border-box;padding:11px 14px;border:1.5px solid #E1E5F1;border-radius:12px;font-family:DM Sans,sans-serif;font-size:14px;outline:none"></div>' +
          '</div>' +

          '<div class="bm-row-dates">' +
            '<div style="min-width:0;width:100%;box-sizing:border-box"><label style="font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:#6B7A99;display:block;margin-bottom:6px">Дата заезда <span style="color:#E05252">*</span></label>' +
              '<input id="bm-checkin" type="hidden"><div id="bm-checkin-disp" style="width:100%;box-sizing:border-box;padding:11px 14px;border:1.5px solid #E1E5F1;border-radius:12px;font-family:DM Sans,sans-serif;font-size:14px;color:#AFB9D0;background:white;cursor:pointer;min-height:46px;display:flex;align-items:center">Выберите дату ↓</div></div>' +
            '<div style="min-width:0;width:100%;box-sizing:border-box"><label style="font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:#6B7A99;display:block;margin-bottom:6px">Дата выезда</label>' +
              '<input id="bm-checkout" type="hidden"><div id="bm-checkout-disp" style="width:100%;box-sizing:border-box;padding:11px 14px;border:1.5px solid #E1E5F1;border-radius:12px;font-family:DM Sans,sans-serif;font-size:14px;color:#AFB9D0;background:#F4F6FB;min-height:46px;display:flex;align-items:center">Рассчитывается</div></div>' +
          '</div>' +
        ...
        '<div id="bm-success-screen" style="display:none;text-align:center;padding:20px 0">' +
          ...
          '<div style="background:rgba(62,112,87,0.07);border-radius:14px;padding:16px 18px;text-align:left;margin-bottom:20px">' +
            '<div style="font-size:13px;color:#2E384D;line-height:1.6">✅ Уведомление о получении заявки и детализация отправлены в чат.</div>' +
  ...
  return {
    ...
    open: function(opts) {
      ...
      if (opts.dateIn && el('bm-checkin')) {
        ...
        var dI = new Date(opts.dateIn + 'T00:00:00');
        var fI = dI.getDate().toString().padStart(2,'0') + '.' + (dI.getMonth()+1).toString().padStart(2,'0') + '.' + dI.getFullYear();
        var dispI = el('bm-checkin-disp'); if (dispI) { dispI.textContent = fI; dispI.style.color = '#2E384D'; }
      }
      ...
      setTimeout(function(){ BookingModal._initPhoneMask(); }, 50);
    ...
    _phoneMask: null,
    _initPhoneMask: function() {
      var inp = el('bm-phone');
      if (!inp) return;
      if (this._phoneMask) { this._phoneMask.destroy(); this._phoneMask = null; }
      if (typeof IMask === 'undefined') return;
      this._phoneMask = IMask(inp, { mask: '+7(000) 000-00-00', lazy: false, placeholderChar: '_' });
    },
    _fmtPhone: function(inp) { /* заменено IMask */ },
    ...
    _recalc: function() {
      ...
      // Обновляем дисплеи дат
      var dIn = new Date(dateIn + 'T00:00:00');
      var dOut2 = new Date(dateOut + 'T00:00:00');
      var fmtIn = dIn.getDate().toString().padStart(2,'0') + '.' + (dIn.getMonth()+1).toString().padStart(2,'0') + '.' + dIn.getFullYear();
      var fmtOut = dOut2.getDate().toString().padStart(2,'0') + '.' + (dOut2.getMonth()+1).toString().padStart(2,'0') + '.' + dOut2.getFullYear();
      var dispIn = el('bm-checkin-disp'); if (dispIn) { dispIn.textContent = fmtIn; dispIn.style.color = '#2E384D'; }
      var dispOut = el('bm-checkout-disp'); if (dispOut) { dispOut.textContent = fmtOut; dispOut.style.color = '#2E384D'; }
    ...
    _calRender: function() {
      ...
      for (var d=1;d<=daysInMonth;d++) {
        ...
        var dtStr=dt.getFullYear()+'-'+String(dt.getMonth()+1).padStart(2,'0')+'-'+String(d).padStart(2,'0');
        var isPast=dt<new Date(new Date().toDateString());
        var bg=isOcc?'rgba(224,82,82,0.12)':isSel?'rgba(62,112,87,0.15)':'transparent';
        var cursor=isOcc||isPast?'default':'pointer';
        var onclick=(!isOcc&&!isPast)?'onclick="BookingModal._calSelect(\'' +dtStr+ '\')"':'';
        cells+='<div '+onclick+' style="text-align:center;padding:4px 2px;border-radius:6px;font-size:12px;font-weight:'+(isToday?'700':'400')+';background:'+bg+';color:'+(isOcc||isPast?'#AFB9D0':'#2E384D')+';cursor:'+cursor+'">'+d+'</div>';
      }
    ...
    _calSelect: function(dateStr) {
      var inp = el('bm-checkin');
      inp.value = dateStr;
      inp.style.borderColor = '';
      // Обновляем дисплей
      var d = new Date(dateStr + 'T00:00:00');
      var fmt = d.getDate().toString().padStart(2,'0') + '.' + (d.getMonth()+1).toString().padStart(2,'0') + '.' + d.getFullYear();
      var disp = el('bm-checkin-disp');
      if (disp) { disp.textContent = fmt; disp.style.color = '#2E384D'; }
      this._recalc();
      this._calRender();
      // Обновляем дисплей выезда
      var out = el('bm-checkout').value;
      if (out) {
        var dOut = new Date(out + 'T00:00:00');
        var fmtOut = dOut.getDate().toString().padStart(2,'0') + '.' + (dOut.getMonth()+1).toString().padStart(2,'0') + '.' + dOut.getFullYear();
        var dispOut = el('bm-checkout-disp');
        if (dispOut) dispOut.textContent = fmtOut;
      }
    },
    ...
    _submit: function() {
      ...
      if (!dateIn){var dDisp=el('bm-checkin-disp');if(dDisp)dDisp.style.border='1.5px solid #E05252';ok=false;}
      ...
      pricePromise.then(function(pc){
        ...
        fetch(cfg.appsUrl+'/bookings',{method:'POST',headers:{'Content-Type':'application/json'},
          ...
          if (d.ok||d.id) {
            ...
            fetch(cfg.appsUrl+'/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'new_booking',booking_id:bookingId,obj_id:state.objId,name:name,phone:phone,date_in:dateIn,date_out:dateOut,nights:nights,amount:amount})}).catch(function(){});
            var info='📋 Новая заявка от '+name+' ('+phone+')\n📅 '+dateIn+' — '+dateOut+', '+nights+' '+nWord+breakdown+'\n💰 Итого: '+amount.toLocaleString('ru')+' ₽';
            if (comment) info+='\n💬 '+comment;
            fetch(cfg.appsUrl+'/chat/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contact:name+' '+phone,dialog:info})}).catch(function(){});
            // Отправляем сообщение в чат на сайте и показываем красную точку
            BookingModal._sendChatMessage(name, dateIn, dateOut, nights, amount, breakdown, nWord, comment);
          }
        }).catch(function(){alert('Ошибка отправки. Попробуйте ещё раз.');});
      });
    },

    _sendChatMessage: function(name, dateIn, dateOut, nights, amount, breakdown, nWord, comment) {
      // Формируем текст сообщения для чата на сайте
      var msg = '✅ Ваша заявка получена!\n\n';
      msg += '📅 ' + dateIn + ' — ' + dateOut + ', ' + nights + ' ' + nWord;
      if (breakdown) msg += '\n\nДетализация:' + breakdown;
      msg += '\n\n💰 Итого: ' + amount.toLocaleString('ru') + ' ₽';
      if (comment) msg += '\n💬 ' + comment;
      msg += '\n\nМы свяжемся с вами для подтверждения.';

      // Добавляем в чат через ChatWidget
      if (typeof ChatWidget !== 'undefined') {
        setTimeout(function() {
          if (!ChatWidget._isOpen()) ChatWidget.toggle();
          var msgHtml = msg.replace(/\n/g, '<br>');
          ChatWidget._addMsg(msgHtml, 'bot', true);
          setTimeout(function(){
            var privacyMsg = 'Укажите, пожалуйста, предпочтительный способ связи с вами: телефон, WhatsApp, Telegram или другой.<br><br>Отправляя контакт, вы подтверждаете согласие на <a href="https://aparts-baltika.ru/privacy.html" target="_blank" style="color:#3E7057;text-decoration:underline">обработку персональных данных</a>.';
            ChatWidget._addMsg(privacyMsg, 'bot', true);
          }, 800);
        }, 600);
      }
      // Показываем красную точку на иконке чата
      var btn = document.getElementById('chat-widget-btn');
      if (btn) {
        btn.classList.add('has-unread');
        // Убираем точку когда чат открыт
        var origToggle = window.cwToggle;
        if (typeof origToggle === 'function') {
          window.cwToggle = function() {
            origToggle();
            btn.classList.remove('has-unread');
            window.cwToggle = origToggle; // восстанавливаем оригинал
          };
        }
      }
    },
