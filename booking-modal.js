...
var BookingModal = (function() {
      ...
      '<div id="bm-modal" style="background:white;border-radius:28px;width:100%;max-width:480px;max-height:90vh;overflow-y:auto;padding:32px;position:relative;box-shadow:0 24px 80px rgba(46,56,77,0.2)">' +
        ...
        '<div id="bm-form-screen">' +
          ...
          '<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:#F4F6FB;border-radius:10px;border:1px solid #E1E5F1;margin-bottom:12px">' +
            ...
            '<input id="bm-nights" type="number" value="3" min="1" max="90" oninput="BookingModal._recalc();BookingModal._calRender()" style="width:56px;text-align:center;font-weight:700;font-family:Manrope,sans-serif;font-size:16px;border:1.5px solid #E1E5F1;border-radius:8px;padding:4px 8px;outline:none">' +
            '<button type="button" onclick="BookingModal._nights(1)" style="width:28px;height:28px;border-radius:6px;border:1.5px solid #E1E5F1;background:white;font-size:16px;cursor:pointer;flex-shrink:0;display:flex;align-items:center;justify-content:center">+</button>' +
          ...
          '<p style="text-align:center;font-size:11px;color:#6B7A99;margin-top:10px;line-height:1.5">Бронирование вступает в силу после подтверждения управляющего.<br>Нажимая кнопку, вы соглашаетесь с обработкой персональных данных.</p>' +
        '</div>' +
  ...
  return {
    ...
    _nights: function(delta) {
      var n = el('bm-nights');
      n.value = Math.max(1, Math.min(90, (parseInt(n.value)||1)+delta));
      this._recalc();
      this._calRender();
    },
    ...
    _submit: function() {
      ...
      if (!name){el('bm-name').style.borderColor='#E05252';return;}
      if (phone.replace(/\D/g,'').length!==11){el('bm-phone').style.borderColor='#E05252';return;}
      if (!dateIn){el('bm-checkin').style.borderColor='#E05252';return;}
