...
var BookingModal = (function() {
  ...
  return {
    ...
    _sendChatMessage: function(name, dateIn, dateOut, nights, amount, breakdown, nWord, comment) {
      ...
      if (btn) {
        var dot = document.getElementById('cw-unread-dot');
        if (!dot) {
          dot = document.createElement('div');
          dot.id = 'cw-unread-dot';
          dot.style.cssText = 'position:absolute;top:0;right:0;width:12px;height:12px;border-radius:50%;background:#E05252;border:2px solid white';
          btn.style.position = 'relative';
          btn.appendChild(dot);
        }
        dot.style.display = 'block';
      }
      // Скрываем точку когда чат открыт
      if (typeof cwToggle !== 'undefined') {
        var origToggle = window.cwToggle;
        window.cwToggle = function() {
          origToggle();
          var d = document.getElementById('cw-unread-dot');
