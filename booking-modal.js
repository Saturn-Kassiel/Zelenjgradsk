      ...
      pricePromise.then(function(pc){
        ...
        fetch(cfg.appsUrl+'/bookings',{method:'POST',headers:{'Content-Type':'application/json'},
          ...
          if (d.ok||d.id) {
            ...
            BookingModal._sendChatMessage(name, dateIn, dateOut, nights, amount, breakdown, nWord, comment, state.objName);
          }
        }).catch(function(){alert('Ошибка отправки. Попробуйте ещё раз.');});
      });
    },

    _sendChatMessage: function(name, dateIn, dateOut, nights, amount, breakdown, nWord, comment, objName) {
      // Формируем текст сообщения для чата на сайте
      var inFmt = dateIn ? dateIn.split('-').reverse().join('.') : dateIn;
      var outFmt = dateOut ? dateOut.split('-').reverse().join('.') : dateOut;
      var msg = '✅ ' + name + ', ваша заявка получена!\n\n';
      if (objName) msg += '🏠 ' + objName + '\n';
      msg += '📅 ' + inFmt + ' — ' + outFmt + ', ' + nights + ' ' + nWord;
