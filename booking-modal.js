...
var BookingModal = (function() {
  ...
  return {
    ...
    _submit: function() {
      ...
      var digits = (BookingModal._phoneMask ? BookingModal._phoneMask.unmaskedValue : phone.replace(/\D/g,''));
      if (digits.length!==11){
    ...
    _sendChatMessage: function(name, dateIn, dateOut, nights, amount, breakdown, nWord, comment) {
      ...
      // Добавляем в чат как сообщение от бота
      if (typeof cwAddMsg === 'function') {
        var msgHtml = msg.replace(/\n/g, '<br>');
        cwAddMsg(msgHtml, 'bot', true);
        // Второе сообщение с просьбой указать способ связи
        setTimeout(function(){
