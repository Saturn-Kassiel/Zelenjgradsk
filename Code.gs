  ...
  try {
    ...
    if (action === 'chat') {
      ...
      try {
        ...
        // Объекты читаем с REST API (из админки сайта)
        const objResponse = UrlFetchApp.fetch(API_BASE + '/objects', {
          muteHttpExceptions: true
        });
        const objData = JSON.parse(objResponse.getContentText());
        let objectsText = '';
        (objData.data || []).forEach((p, i) => {
