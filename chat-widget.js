...
var ChatWidget = (function() {
  ...
  var cfg = { appsUrl: '', chatUrl: '', objId: '1', getObjId: null };
  var cwHistory = [];
  var cwOpen = false;
...
.cw-bubble {
  ...
  font-family: DM Sans, sans-serif; font-size: 14px; line-height: 1.5; word-break: break-word;
}
...
`;
  ...
  function saveHistory() {
    try { localStorage.setItem('cw_history_' + getObjId(), JSON.stringify(cwHistory.slice(-20))); } catch(e) {}
  }

  function loadHistory() {
    try {
      var saved = localStorage.getItem('cw_history_' + getObjId());
  ...
  // ── QUEUE ──
  function checkQueue() {
    fetch(cfg.appsUrl + '/chat-queue/' + getObjId())
      .then(function(r) { return r.json(); })
      .then(function(d) {
        if (d.ok && d.messages && d.messages.length) {
          d.messages.forEach(function(msg, i) {
            setTimeout(function() {
  ...
  return {
    ...
    toggle: function() {
      ...
      if (cwOpen) {
        ...
        if (rendered === 0) {
          this._renderHistory();
        } else if (rendered - 1 < cwHistory.length) {
          for (var i = rendered - 1; i < cwHistory.length; i++) {
    ...
    clear: function() {
      cwHistory = [];
      try { localStorage.removeItem('cw_history_' + getObjId()); } catch(e) {}
    ...
    send: async function() {
      ...
      if (cwWaitingContact) {
        ...
        fetch(cfg.chatUrl, {
          ...
          body: JSON.stringify({ action: 'notify', contact: text, dialog: dialogLines })
        }).catch(function() {});
        return;
      }
      ...
      try {
        var res = await fetch(cfg.chatUrl, {
          ...
          headers: { 'Content-Type': 'text/plain' },
          redirect: 'follow',
          body: JSON.stringify({ action: 'chat', messages: cwHistory })
        });
    ...
    onBookingSuccess: function(name, objName, dateIn, dateOut, nights) {
      ...
      cwHistory.push({ role: 'assistant', content: msg });
      saveHistory();
      if (cwOpen) { addMsg(msg, 'bot'); }
      else { var btn = el('chat-widget-btn'); if (btn) btn.classList.add('has-unread'); }
    }
  };
